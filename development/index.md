...
DBGV_ON((varName)(function())(3)); // prints: "varName : varValue | function() : returnValue | 3 : 3"
```
...
- <a id="UTILITIES_DEFENSIVE"></a>**Defensive programming**: check
...
error::exception(message); // throws a run-time error (use _only_ if something can be done about the error)
```
...
- <a id="UTILITIES_IO"></a>**Input/Output**:
  ...
  - **Properties** (simulation, grid generation, ...):
      ...
      ```c++
      io::Properties myProperties; // creates property container
      io::insert_property<Num>(myProperties,"dx",3.0); // inserts property of type Num and name "dx" into myProperties
      auto dx = io::read<Num>(myProperties,"dx"); // reads property of type Num and name "dx"
      io::read(myProperties,"dx",dx); // reads property of type decltype(dx) and name "dx"
      ```
  ...
  - prefer [concept-based run-time
    ...
    [faster](http://probablydance.com/2013/01/13/a-faster-implementation-of-stdfunction/)):
      ...
      struct Interface {
        ...
        /// The easiest way to provide value semantics is to use a model sub-struct with type-erasure.
      };
      ```
