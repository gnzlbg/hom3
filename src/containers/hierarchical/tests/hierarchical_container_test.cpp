/// \file \brief Tests for container::hierarchical
/// Includes:
...
template<SInd nd> io::Properties small_grid(const SInd minRefLevel) {
  ...
  auto properties  = grid::helpers::cube::properties<nd>
      ({NumA<nd>::Constant(0), NumA<nd>::Constant(1)}, minRefLevel);
  io::insert_property<Boundaries>(properties, "boundaries", boundaries);
  return properties;
}
...
TEST(hierarchical_container_test, test_child_position_in_parent) {
  ...
  for (auto child : small3DGrid.childs(NodeIdx{0})) {
    EXPECT_EQ(expected_positions_0(child),
              small3DGrid.position_in_parent(child));
  }

  // if the child has no parent you should expect a death in debug mode
  EXPECT_DEBUG_DEATH_(small3DGrid.position_in_parent(NodeIdx{0}));
}
...
TEST(hierarchical_container_test, test_3D_strict_samelvl_nghbrs) {
  ...
  EXPECT_EQ(decltype(boost::distance(rootNghbrs))(2 * nd),
            boost::distance(rootNghbrs));
  ...
  // checks nghbrs
  auto check_nghbrs
    = [](const NodeIdx nId, const std::array<Ind, 6>& nghbrsNid) {
...
TEST(hierarchical_container_test, test_2D_strict_samelvl_nghbrs) {
  ...
  EXPECT_EQ(boost::distance(rootNghbrs),
            decltype(boost::distance(rootNghbrs))(2 * nd));
  ...
  // checks nghbrs
  auto check_nghbrs
    = [](const NodeIdx nIdx, const std::array<Ind, 4>& shouldNghbrId) {
