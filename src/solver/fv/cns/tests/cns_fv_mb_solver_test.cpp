...
(grid::Grid<nd>* grid, const Num timeEnd, const Num ReInfinity,
  typename CNSSolver<nd>::InitialDomain initialDomain
    = [](const NumA<nd>) { return true; },
...
TEST(cns_fv_solver, flow_past_sphere) {
 ...
 /// Create solver
  auto cnsSolver = CNSSolver<nd> {
    cnsSolverIdx,
    cns_properties<nd>(&grid, 300.0, 200.)
  };
  ...
  std::function<Num(SInd)> acceleration = [&](SInd d) {
  ...
  auto mBc = cns_physics::bc::moving_wall::AdiabaticNoSlip<CNSSolver<nd>>{
    cnsSolver, moving_sphere, velocity, acceleration
  };
