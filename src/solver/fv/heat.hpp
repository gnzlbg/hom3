...
template<SInd nd, class Solver> struct Physics {
  ...
  template<class U> inline Num& T(const CellIdx lId)       { return Q_<U>(lId,V::T()); }
  template<class U> inline Num  T(const CellIdx lId) const { return Q_<U>(lId,V::T()); }
  ...
  template<class U> inline Num compute_dt(const CellIdx lId) const {
  ...
  template<class U> inline void check_variables(const CellIdx lId) const {
 ...
 private:
  ...
  (const CellIdx lId, const CellIdx rId, const SInd, const Num dx,
...
template<class Solver> struct Dirichlet : boundary::Condition {
  ...
  void apply(AnyRange<CellIdx> ghost_cells) const override {
    for(auto ghostCellId : ghost_cells) {
      ...
      s.cells().lhs(ghostCellId,0) = 2.0 * value(bndryCellId,0) - s.cells().lhs(bndryCellId,0);
      s.cells().rhs(ghostCellId,0) = 2.0 * value(bndryCellId,0) - s.cells().rhs(bndryCellId,0);
    }
  }

  std::function<Num(CellIdx,SInd)> value;
  Solver& s;
};
...
template<class Solver> struct Neumann : boundary::Condition {
  ...
  void apply(AnyRange<CellIdx> ghost_cells) const override {
    for(auto ghostCellId : ghost_cells) {
      ...
      std::tie(bndryCellId,std::ignore) = s.boundary_cell_id(ghostCellId);
      s.cells().lhs(ghostCellId,0) = s.cells().lhs(bndryCellId,0) - dx * value(bndryCellId,0);
