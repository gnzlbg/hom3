#ifndef MISC_DEBUG_HPP
#define MISC_DEBUG_HPP
////////////////////////////////////////////////////////////////////////////////
/// \file \brief Implements helpful macros for happy debugging (DBG,DBGV).
///
...
/// \brief If on is true, stream arguments to std::cerr, otherwise does nothing.
///
/// \todo replace std::cerr with something better
///@{
/// \brief dout N-recursion
template <class H> void dout(const bool on, H&& msg) { ///< Base case (N Recursion)
  if(on) { ///< If on is false at compile time dout is removed.
    std::cerr << msg << " ";
  }
}
/// \brief dout (N-1)-recursion
...
#ifndef NDEBUG
/// \brief DOUT forwards the call to debug::dout if NDEBUG is not defined
#define DOUT(...)                                             \
  do {                                                        \
    if (true) {                                               \
...
#ifndef NDEBUG
/// \brief Like DOUT if condition is true, otherwise does nothing
...
/// \brief Specifies the output format for DBGV
#define DBG_FORMAT(x) "|", #x":", (x)

/// \brief "Closure" for Boost's PP for_each
#define MEMBER(r, data, i, elem) BOOST_PP_COMMA_IF(i) DBG_FORMAT(elem)

/// \brief Given a list of variables enclosed in parentheses, it
/// forwards to DOUT a sequence of variable names and variable values
/// using the format specified in DBG_FORMAT
#define DOUTV(on, members)                                      \
  DOUT(on,                                                      \
       BOOST_PP_SEQ_FOR_EACH_I( MEMBER,, members )              \
       )

#define DOUTV_IF(condition, on, members)                        \
  DOUT_IF(condition, on,                                        \
       BOOST_PP_SEQ_FOR_EACH_I( MEMBER,, members )              \
       )

/// \brief Debug macros independent of ENABLE_DEBUG_
///@{
#define DBG_ON(...)   do { DOUT(1,"#",__VA_ARGS__);  } while (false)
#define DBG_OFF(...)  do { DOUT(0,"#",__VA_ARGS__);  } while (false)
#define DBG_IF_ON(pred,...)   do { DOUT_IF(pred,1,"#",__VA_ARGS__);  } while (false)
#define DBG_IF_OFF(pred,...)  do { DOUT_IF(pred,0,"#",__VA_ARGS__);  } while (false)
#define DBGV_ON(...)  do { DOUTV(1,__VA_ARGS__); } while (false)
#define DBGV_OFF(...) do { DOUTV(0,__VA_ARGS__); } while (false)
#define DBGV_IF_ON(pred,...)  do { DOUTV_IF(pred,1,__VA_ARGS__); } while (false)
#define DBGV_IF_OFF(pred,...) do { DOUTV_IF(pred,0,__VA_ARGS__); } while (false)
///@}
///@}
